...
class AnalysisRepositoryTest {
    ...
    private fun ret(value: ExpressionElement? = null): eu.nitok.jitsu.compiler.graph.elements.Return =
        eu.nitok.jitsu.compiler.graph.elements.Return(value, dummyLocation).modularized()

    private fun call(name: String, args: List<ExpressionElement> = emptyList()): FunctionCall =
        FunctionCall(located(name), args, dummyLocation).modularized()

    @Test
    fun `single non-recursive function gets summary`() {
        val foo = buildFunction("foo", i32, instructions = listOf(ret(constInt(5))))
    ...
    fun `two independent functions both get summaries`() {
        val foo = buildFunction("foo", i32, instructions = listOf(ret(constInt(5))))
        val bar = buildFunction("bar", i32, instructions = listOf(ret(constInt(10))))
    ...
    fun `linear call chain foo calls bar - bar analyzed before foo`() {
        val bar = buildFunction("bar", i32, instructions = listOf(ret(constInt(5))))
    ...
    fun `scc ordering - bar is leaf, foo calls bar, foo uses bar summary`() {
        val bar = buildFunction("bar", i32, instructions = listOf(ret(constInt(42))))
    ...
    fun `variable summaries stored for analyzed functions`() {
        val decl = VariableDeclaration(
            ...
            initialValueElement = constInt(7)
        )
