...
import com.niton.parsers.grammar.gen.*;
import com.niton.parser.token.TokenPattern;

import java.io.IOException;

import static com.niton.parsers.grammar.GrammarGrammarName.*;
import static com.niton.parsers.grammar.GrammarTokens.*;
...
public class GrammarParser extends Parser<GrammarFileContent> {
	...
	public GrammarParser() {
		super(grammarFile);
		getTokenizer().setTokens(GrammarTokens.values());
		...
		for (RootGrammar gram : g.getGrammars()) {
			...
			for (SubGrammar r : gram.getChain()) {
				...
				if (r.getItem().getType().equals(GRAMMAR_REFERENCE.getName())) {
					com.niton.parsers.grammar.gen.GrammarReference check = new com.niton.parsers.grammar.gen.GrammarReference(
							(SuperNode) r.getItem().getRes());
					...
					if (arr.getItems().size() == 0) {
						...
						for (int i = 0; i < grms.length; i++) {
							...
							grms[i] = Grammar.reference(new com.niton.parsers.grammar.gen.GrammarReference(
									(SuperNode) arr.getItems()
									               .get(i)
									               .getItem()
						...
						for (int i = 0; i < grms.length; i++) {
							...
							grms[i] = Grammar.tokenReference(new com.niton.parsers.grammar.gen.TokenReference(
									(SuperNode) arr.getItems()
									               .get(i)
									               .getItem()
									               .getRes()).getTokenName());
