...
public class ManualTest {
	public static void main(String[] args) throws IOException {
		...
		var           reduced = result.unwrap().reduce("expression").orElseThrow();
		serveAstHtml(reduced);
		System.out.println(reduced.format());
		System.out.println(result.unwrap().joinTokens() + " = " + calculateExpression(reduced));
	}
	...
	private static int calculateOperator(int startValue, ReducedNode op)  {
		...
		switch (type) {
			...
			case "Multiplicative":
				...
				break;
			default:
				throw new IllegalStateException("No operation type " + type + " known");
		}
	...
	private static int applyOperation(int startValue, String operator, int expressionVal)
		...
		switch (operator) {
			...
			case "/":
				...
				break;
			default:
				throw new IllegalStateException("No operator " + operator + " known");
		}
		return startValue;
	}
