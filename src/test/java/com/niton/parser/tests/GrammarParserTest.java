...
class GrammarParserTest {
	...
	void parseFile() {
		assertDoesNotThrow(()->{
			...
			GrammarFileContent result = parser.parse(file);
		});
	}
...
//				System.out.println("Grammar Pure :\n "+((SuperGrammarResult)p.parsePlain(s)).toString(3));
//			}
//			GrammarResult result =  p.parsePlain(s);
//			GrammarFile file = new GrammarFile((SuperGrammarResult)result);
...
//				}
//			}
//			assertEquals(0, expectedGrammarNames.size(),"Some grammars where not parsed ("+expectedGrammarNames+")");
//
//			GrammarMatcher exec =p.getReference().get(p.getRoot()).createExecutor();
//			exec.parse(p.getTokenizer().parse(s), p.getReference());
