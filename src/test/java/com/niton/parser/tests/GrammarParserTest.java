...
class GrammarParserTest {
	...
	private static void test(String s,
		...
		assertDoesNotThrow(() -> {
			...
			if (debug) {
				...
				System.out.println("Grammar Pure :\n " + ((SuperGrammarResult) p.parsePlain(s)).toString(
						3));
			}
			...
			if (file.getHead() != null && file.getHead().getTokenDefiners() != null) {
				for (IgnoringTokenDefiner string : file.getHead().getTokenDefiners()) {
					assertTrue(tokenNamesExpected.contains(string.getContent().getName()),
			...
			if (file.getGrammars() != null) {
				for (com.niton.parser.specific.grammar.gen.RootGrammar string : file.getGrammars()) {
	...
	void parseFile() {
		assertDoesNotThrow(() -> {
			...
			byte[]             file   = IOUntility.readOutOfJarFile("Example.gmr");
