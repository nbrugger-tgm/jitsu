...
import com.niton.parser.specific.grammar.gen.OrOperation;
    ...
    public static GrammarReference gram = new GrammarReferenceMap()
            ...
            .map(
                    Grammar.build("matchOperation")
                            .matchTokenOptional(GrammarTokens.NEGATE, "anyExcept")
                            .ignoreToken(GrammarTokens.WHITESPACE)
    ...
    public GrammarResult convert(GrammarObject o) throws ParsingException {
        ...
        for (com.niton.parser.specific.grammar.gen.Grammar gram : g.getGrammars()) {
            ...
            for (Rule r : gram.getChain()) {
                line++;
                if (r.getOperation().getType().equals("matchOperation")) {
                    ...
                    if (op.getCheck().getType().equals("grammarLiteral")) {
                        //GRAMMAR
                    ...
                    } else if (op.getCheck().getType().equals("tokenSubst")) {
                ...
                } else if (r.getOperation().getType().equals("orOperation")) {
                    OrOperation or = new OrOperation(r.getOperation());
                    String name = null;
                    if (or.getAssignment() != null)
                        name = or.getAssignment().getName();
                    String[] array = new String[or.getItems().size()];
                    if (or.getItems().get(0).getItem().getType().equals("grammarLiteral")) {
                        for (int i = 0; i < array.length; i++) {
                            GrammarLiteral lit = new GrammarLiteral(or.getItems().get(i).getItem());
                            array[i] = lit.getGrammarName();
                        }
                        gr.matchAny(name, array);
                    } else if (or.getItems().get(0).getItem().getType().equals("grammarLiteral")) {
                        for (int i = 0; i < array.length; i++) {
                            TokenSubst lit = new TokenSubst(or.getItems().get(i).getItem());
                            array[i] = lit.getTokenName();
                        }
                        gr.matchAnyToken(name, array);
                    }
