...
public class JPGenerator {
	...
	public void generate(GrammarReference reference, Tokenable[] tokenable)
	...
	private void generateChainGrammarClass(ChainGrammar grammar, GrammarReference ref)
	throws IOException {
		FieldSpec wrappedResult = FieldSpec.builder(SuperGrammarResult.class, "result")
			...
			if (subGrammar instanceof MultiGrammar) {
				getter.add(
						MethodSpec.methodBuilder("get" + javaFy(propertyName))
						          ...
						          .addStatement("ResultResolver.setResolveAny(" + !returnType.toString()
						                                                                     .equals(TypeName.get(
								                                                                     AnyGrammarResult.class)
						                                                                                     .toString()) + ")")
						          .addStatement(
								          "return ($T) $T.getReturnValue(result.getObject($S))",
								          returnType,
				...
				getter.add(MethodSpec.methodBuilder("get" + javaFy(propertyName))
				                     .returns(returnType)
				                     .addModifiers(PUBLIC)
				                     .beginControlFlow("if(result.getObject($S) == null)",
				                                       propertyName)
				...
				getter.add(MethodSpec.methodBuilder("get" + javaFy(propertyName))
				                     .returns(returnType)
				                     .addModifiers(PUBLIC)
				                     .addStatement("return ($T) result.getObject($S)",
				                                   returnType,
				                                   propertyName)
				                     .build());
				...
				getter.add(MethodSpec.methodBuilder("get" + javaFy(propertyName))
				                     ...
				                     .addModifiers(PUBLIC)
				                     .addStatement(
						                     "if($T.getReturnValue(result.getObject($S)) == null)" +
								                     "return null;" +
								                     "return new $T(($T)$T.getReturnValue(result.getObject($S)))",
						                     ResultResolver.class,
						                     propertyName,
						                     returnType,
						                     SuperGrammarResult.class,
						                     ResultResolver.class,
						                     propertyName)
				                     .build());
			}
	...
	private void generateTokenFile(Tokenable[] tokenable) throws IOException {
		Builder tokenFile = TypeSpec
				...
				.addMethod(MethodSpec.constructorBuilder()
				                     ...
				                     .addStatement("this.pattern = pattern")
				                     .build());
        for (Tokenable t : tokenable) {
            tokenFile = tokenFile.addEnumConstant(t.name(),
                                                  TypeSpec.anonymousClassBuilder("$S", t.pattern())
                                                          .build());
        }
	...
	public TypeName getReturnType(Grammar g, GrammarReference ref) {
		...
		if (g instanceof MultiGrammar) {
            if (((MultiGrammar) g).getGrammars().length > 0) {
                if (Arrays.stream(((MultiGrammar) g).getGrammars())
                          .map(gr -> getReturnType(resolve(gr, ref), ref).toString())
                          .distinct()
                          .count() == 1) {
        ...
        if (g instanceof RepeatGrammar) {
            return ParameterizedTypeName.get(ClassName.get(List.class),
                                             getReturnType(resolve(((RepeatGrammar) g).getCheck(),
                                                                   ref), ref));
        }
		return null;
	}
