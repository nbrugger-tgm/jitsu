	...
	public List<AssignedToken> tokenize(String content) throws ParsingException {
		...
		for (AssignedToken assignedToken : tokens) {
			if (assignedToken.start > last) {
				AssignedToken undef = new AssignedToken(content.substring(last,
				                                                          assignedToken.start),
				                                        Tokenizer.UNDIFINED,
				                                        "UNDEFINED");
				undef.start = last;
				...
				throw new ParsingException("Tokens overlapping: " + assignedToken + " overlaps previous Token! Last token ended at " + last + " and this token startet at " + assignedToken.start + " (" + content
						.substring(last - 5, last + 5) + ")");
			}
